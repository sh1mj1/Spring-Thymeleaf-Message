...
label.item=상품
label.item.id=상품 ID
label.item.itemName=상품명2

label.item.price=가격2
label.item.quantity=수량2
page.items=상품 목록
page.item=상품 상세
    ...
    <div class="row">
        ...
        <div class="col">
            <button class="w-100 btn btn-secondary btn-lg"
                    onclick="location.href='items.html'"
                    th:onclick="|location.href='@{/message/items}'|"
                    type="button" th:text="#{page.items}">목록으로
            </button>
        </div>
    </div>
</div> <!-- /container -->
...
실행 후 잘 동작하는지 확인하기 위해 messages.properties 파일의 내용을 `가격`을 `가격2` 처럼 변경해서 확인해봅시다.

> 참고 - 파라미터는 다음과 같이 사용할 수 있습니다.
`hello.name=안녕 {0}`
